...
/**
 * ISO 3166-1 country codes. Edge stores countries as alpha-3.
 *
 * The table mirrors the countries the backend's geo database knows about
 * (countries-states-cities-database 2.5, loaded by `Core.Release.refresh_geo_db`).
 * That includes Kosovo's user-assigned XK/XKX. The backend also accepts country
 * names, but this package only takes codes, so a typo fails here rather than
 * matching the wrong country.
 */
final class Countries
{
    ...
    /**
     * Normalises an alpha-2 or alpha-3 code, in any case, to alpha-3.
     *
     * @throws InvalidRequestException when the code is not a known country
     */
    public static function toAlpha3(string $code): string
    {
        $normalised = strtoupper(trim($code));

        if (strlen($normalised) === 2 && isset(self::ALPHA2_TO_ALPHA3[$normalised])) {
            return self::ALPHA2_TO_ALPHA3[$normalised];
        }

        if (strlen($normalised) === 3 && in_array($normalised, self::ALPHA2_TO_ALPHA3, true)) {
            return $normalised;
        }

        throw new InvalidRequestException(sprintf('"%s" is not a recognised ISO 3166-1 country code.', $code));
    }
}
